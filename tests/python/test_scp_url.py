...
from liburlparser import ScpUrl
...
def test_is_scp_like_true(s):
    assert ScpUrl.is_scp_like(s)


@pytest.mark.parametrize("s", NOT_SCP_LIKE)
def test_is_scp_like_false(s):
    assert not ScpUrl.is_scp_like(s)


def test_normalize_basic_case():
    assert ScpUrl.normalize("git@github.com:user/repo.git") == \
        "ssh://git@github.com/user/repo.git"


def test_normalize_leaves_normal_url_unchanged():
    assert ScpUrl.normalize("https://github.com/user/repo.git") == \
        "https://github.com/user/repo.git"


def test_constructor_parses_scp_like_as_ssh_url():
    s = ScpUrl("git@github.com:mohammadraziei/liburlparser.git")
...
def test_constructor_accepts_normal_url_unchanged():
    s = ScpUrl("https://github.com/mohammadraziei/liburlparser.git")
    assert not s.was_scp_like
    assert s.url.protocol == "https"


def test_repr():
    s = ScpUrl("git@github.com:user/repo.git")
